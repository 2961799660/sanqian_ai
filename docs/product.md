...
| GPT Image 2 | OpenAI 系最新版 |

可控项:数量 · 清晰度 · 比例(1:1 / 16:9 / 9:16)· 参考图(最多 5 张)。
...
适合想自己当老板做拓展、开城市代理、做矩阵分销的团队。**两级代理体系,加盟即享销售提成**。
...
> **提成结算**:每成单一份产品,按零售价 × 提成比例 实时结算到代理后台。
> **零售价**:厂商指导价 ¥2280 / ¥3980 / ¥13800,代理可在指导价基础上加价销售,**多收的部分 100% 归代理**。
> **加盟费回本**:初级代理只需卖出 **3 单包搭建** 或 **2 单开源源码** 即可回本;中级代理只需卖出 **2 单包搭建** 或 **1 单源码 + 1 单包搭建** 即可回本。

### 代理共同权益

- ✅ 加盟立即获得 **1 份开源源码授权**(可自用 / 自建演示站,价值 ¥13800)
- ✅ 全套销售物料 —— 产品介绍 PDF · 主视觉长图 · 朋友圈话术 · 商品标题模板 · FAQ 话术包
- ✅ 厂商提供 **售前在线咨询** + 客户 **售后远程部署** 支持
- ✅ 中级代理可发展初级代理,**自动结算级差提成**(中 → 初 差额 1 成)
- ✅ 代理资格 **永久有效**,无年费、无续签

### 加盟流程

1. 联系商务微信 `q2026560558` 沟通需求与等级
2. 缴纳对应等级加盟费,签订代理协议
3. 开通代理后台账号 + 领取销售物料包
4. 立即开始推广,成单结算到账

> 💬 想清楚选哪一级?加微信聊,**按你的渠道资源给推荐方案**。
...
**Q:做代理需要什么条件?**
不需要技术背景。只需对自媒体 / AI 工具 / 私域获客有自己的渠道资源。初级代理(¥8900)送 1 份开源源码,卖 3 单包搭建或 2 单源码即可回本。

**Q:代理可以发展下级吗?**
可以。中级代理(¥12999)发展的初级代理(¥8900)成单时,中级代理自动获得 **1 成级差** 提成(差额结算到中级账户)。两层简单清晰,无复杂传销结构。

**Q:初级和中级怎么选?**
预计月销 5 单内 → 选**初级**(¥8900,提 3 成);
预计月销 5 单以上,或想发展下级代理 → 选**中级**(¥12999,提 4 成 + 级差返佣);
两级都送 1 份开源源码,加盟即回本入口。
